...
const MAX_RAW_SIZE = CHUNK_SIZE * 20; // Increased to 10M to allow full coverage at max zoom-out without blank areas
...
function fetchCandles(offset, limit) {
  console.log(`[fetchCandles] Starting fetch: offset=${offset}, limit=${limit}`);
  // Limit to total candles
  const numCandles = Math.min(limit, TOTAL_CANDLES - offset);
  if (numCandles <= 0) {
    console.log(`[fetchCandles] No candles to fetch: numCandles=${numCandles}`);
    return [];
  }
  ...
  console.log(`[fetchCandles] Fetched ${data.length} candles`);
  return data;
}
...
function aggregateCandles(candles, factor) {
  console.log(`[aggregateCandles] Aggregating: candles.length=${candles.length}, factor=${factor}`);
  if (factor <= 1) return candles;

  const aggregated = [];
  const length = candles.length;
  for (let i = 0; i < length; i += factor) {
    const end = Math.min(i + factor, length);
    if (end - i < 1) break;

    const open = candles[i].open;
    const close = candles[end - 1].close;
    let high = -Infinity;
    let low = Infinity;
    for (let j = i; j < end; j++) {
    ...
    const time = candles[i].time; // Use start time of group

    aggregated.push({ time, open, high, low, close });
  }
  console.log(`[aggregateCandles] Aggregated to ${aggregated.length} candles`);
  return aggregated;
}
...
const DraggableResizableChart = ({ id, left, top, width, height, annotations, moveChart, resizeChart, updateAnnotations }) => {
  ...
  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Updating dimensions: id=${id}, width=${width}, height=${height}`);
  ...
  // Setup chart
  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Setting up chart: id=${id}, currentWidth=${currentWidth}, currentHeight=${currentHeight}`);
    if (chartRef.current) {
      ...
      console.log(`[DraggableResizableChart] Chart created`);
      candleSeries.current = chart.current.addCandlestickSeries({ upColor: '#26a69a', downColor: '#ef5350' });
      console.log(`[DraggableResizableChart] Candlestick series added`);
      lineSeries.current = chart.current.addLineSeries({ color: '#FF0000', lineWidth: 2 });
      console.log(`[DraggableResizableChart] Line series added`);
      ...
      console.log(`[DraggableResizableChart] Initial load: offset=${initialOffset}, limit=${CHUNK_SIZE + 2 * BUFFER_BARS}`);
      ...
      console.log(`[DraggableResizableChart] Initial data loaded: length=${initialData.length}, minIndex=${currentMinIndex.current}, maxIndex=${currentMaxIndex.current}`);
      ...
      console.log(`[DraggableResizableChart] Setting initial visible range: from=${last100[0]?.time}, to=${last100[last100.length - 1]?.time}`);
      ...
      return () => {
        console.log(`[DraggableResizableChart] Cleaning up chart: id=${id}`);
  ...
  const updateChartData = (data) => {
    console.log(`[updateChartData] Updating chart data: length=${data.length}`);
  ...
  const handleRangeChange = () => {
    ...
    console.log(`[handleRangeChange] Range change triggered: now=${now}, lastCall=${lastCall.current}`);
    if (now - lastCall.current < 50) {
      console.log(`[handleRangeChange] Throttled: skipping`);
      return;
    }
  ...
  const updateVisibleData = () => {
    console.log(`[updateVisibleData] Starting update`);
    if (!chart.current) {
      console.log(`[updateVisibleData] No chart instance`);
      return;
    }
    const timeScale = chart.current.timeScale();
    if (!timeScale) {
      console.log(`[updateVisibleData] No timeScale`);
      return;
    }
    const range = timeScale.getVisibleRange();
    if (!range) {
      console.log(`[updateVisibleData] No visible range`);
      return;
    }
    ...
    try {
      ...
      if (rawVisibleBars > 1000000) {
        aggregationFactor *= 2; // Extra aggression for perf at extreme zoom-out
      }

      console.log(`[updateVisibleData] Visible range: from=${from}, to=${to}, duration=${duration}, rawVisibleBars=${rawVisibleBars}, aggregationFactor=${aggregationFactor}`);
      ...
      const clampedFrom = Math.max(from, minTime - bufferTime);
      const clampedTo = Math.min(to, maxTime + bufferTime);
      if (clampedFrom !== from || clampedTo !== to) {
        timeScale.setVisibleRange({ from: clampedFrom, to: clampedTo });
      }
      from = clampedFrom;
      to = clampedTo;
      ...
      console.log(`[updateVisibleData] Calculated indices: minIndex=${minIndex}, maxIndex=${maxIndex}`);
      ...
      console.log(`[updateVisibleData] Effective indices: min=${effectiveMinIndex}, max=${effectiveMaxIndex}, currentMin=${currentMinIndex.current}, currentMax=${currentMaxIndex.current}`);

      if (effectiveMaxIndex < effectiveMinIndex) {
        console.log(`[updateVisibleData] Invalid effective range: updating with available data`);
        const displayData = aggregateCandles(currentRawData.current, aggregationFactor);
      ...
      if (effectiveMinIndex >= currentMinIndex.current && effectiveMaxIndex <= currentMaxIndex.current) {
        console.log(`[updateVisibleData] Current data covers range: slicing and aggregating`);
      ...
      if (effectiveMinIndex < currentMinIndex.current) {
        ...
        console.log(`[updateVisibleData] Loading left: limit=${leftLimit}, from=${leftOffset}`);
      ...
      if (effectiveMaxIndex > currentMaxIndex.current) {
        ...
        console.log(`[updateVisibleData] Loading right: limit=${rightLimit}, from=${rightOffset}`);
      ...
      if (currentRawData.current.length > MAX_RAW_SIZE) {
        ...
        console.log(`[updateVisibleData] Trimming excess raw data: excess=${excess}, loadedLeft=${loadedLeft}, loadedRight=${loadedRight}`);
      ...
      effectiveMinIndex = Math.max(effectiveMinIndex, currentMinIndex.current);
      effectiveMaxIndex = Math.min(effectiveMaxIndex, currentMaxIndex.current);
      ...
      console.log(`[updateVisibleData] Slicing raw data: start=${rawSliceStart}, end=${rawSliceEnd}`);
      ...
      timeScale.subscribeVisibleTimeRangeChange(handleRangeChange);
      console.log(`[updateVisibleData] Update complete`);
    }
  };
  ...
  const loadData = (offset, limit) => { // Changed: Removed async/await to make loadData fully synchronous, avoiding promise delays during panning
    console.log(`[loadData] Starting: offset=${offset}, limit=${limit}`);
    offset = Math.max(0, offset);
    if (offset >= TOTAL_CANDLES) {
      console.log(`[loadData] Offset exceeds total: returning []`);
      return [];
    }
    limit = Math.min(limit, TOTAL_CANDLES - offset);
    if (limit <= 0) {
      console.log(`[loadData] Limit <=0: returning []`);
      return [];
    }
    ...
    while (remaining > 0) {
      ...
      console.log(`[loadData] Processing chunk: key=${chunkKey}, currentOffset=${currentOffset}, remaining=${remaining}`);

      let chunkData;
      if (dataCache.current.has(chunkKey)) {
        console.log(`[loadData] Cache hit for key=${chunkKey}`);
        chunkData = dataCache.current.get(chunkKey);
      } else {
        // Fetch from mock backend
        console.log(`[loadData] Cache miss: fetching chunk from offset=${chunkOffset}`);
        ...
        if (dataCache.current.size > 6) {
          const oldestKey = dataCache.current.keys().next().value;
          console.log(`[loadData] Evicting oldest cache: key=${oldestKey}`);
      ...
      if (take <= 0) {
        console.log(`[loadData] Take <=0: breaking loop`);
        break;
      }

      // Avoid spread for large arrays to prevent stack overflow
      const sliced = chunkData.slice(startInChunk, startInChunk + take);
      console.log(`[loadData] Slicing chunk: start=${startInChunk}, take=${take}, sliced.length=${sliced.length}`);
    ...
    console.log(`[loadData] Loaded total: ${data.length}`);
    return data;
  };

  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Resizing chart: width=${currentWidth}, height=${currentHeight}`);
    if (chart.current) {
  ...
  const drawAnnotations = React.useCallback(() => {
    console.log(`[drawAnnotations] Starting draw: historyIndex=${historyIndex}, annotations.length=${history[historyIndex]?.length}`);
    if (!canvasRef.current || !chart.current || !candleSeries.current) {
      console.log(`[drawAnnotations] Missing refs: canvas=${!!canvasRef.current}, chart=${!!chart.current}, candleSeries=${!!candleSeries.current}`);
      return;
    }
    const ctx = canvasRef.current.getContext('2d');
    const canvasWidth = canvasRef.current.width;
    const canvasHeight = canvasRef.current.height;
    console.log(`[drawAnnotations] Canvas size: width=${canvasWidth}, height=${canvasHeight}`);
    ...
    console.log(`[drawAnnotations] Pane size: width=${paneWidth}, height=${paneHeight}`);
    ...
    if (maxPrice === null || minPrice === null) {
      console.log(`[drawAnnotations] Invalid prices: max=${maxPrice}, min=${minPrice}`);
      return;
    }

    const topY = candleSeries.current.priceToCoordinate(maxPrice);
    const bottomY = candleSeries.current.priceToCoordinate(minPrice);
    console.log(`[drawAnnotations] Price coords: topY=${topY}, bottomY=${bottomY}`);
    ...
    history[historyIndex].forEach(ann => {
      let x1 = timeScale.timeToCoordinate(ann.p1.time);
      let y1 = candleSeries.current.priceToCoordinate(ann.p1.value);
      if (x1 === null || y1 === null) {
        console.log(`[drawAnnotations] Invalid coords for ann: x1=${x1}, y1=${y1}`);
        return;
      }
      // Clamp coords to visible pane to handle out-of-bounds
      x1 = Math.max(0, Math.min(paneWidth, x1));
      y1 = Math.max(topY, Math.min(bottomY, y1));

      if (ann.type === 'text') {
        if (x1 < 0 || x1 > paneWidth || y1 < topY || y1 > bottomY) {
          console.log(`[drawAnnotations] Text out of bounds: x1=${x1}, y1=${y1}`);
          return;
        }
        ctx.font = '14px Arial';
      ...
      let x2 = timeScale.timeToCoordinate(ann.p2.time);
      let y2 = candleSeries.current.priceToCoordinate(ann.p2.value);
      if (x2 === null || y2 === null) {
        console.log(`[drawAnnotations] Invalid coords for ann: x2=${x2}, y2=${y2}`);
        return;
      }
      // Clamp coords to visible pane
      x2 = Math.max(0, Math.min(paneWidth, x2));
      y2 = Math.max(topY, Math.min(bottomY, y2));

      const minX = Math.min(x1, x2);
      const maxX = Math.max(x1, x2);
      const minY = Math.min(y1, y2);
      const maxY = Math.max(y1, y2);
      if (maxX < 0 || minX > paneWidth || maxY < topY || minY > bottomY) {
        console.log(`[drawAnnotations] Annotation out of bounds: minX=${minX}, maxX=${maxX}, minY=${minY}, maxY=${maxY}`);
        return;
      }
    ...
    ctx.restore();
    console.log(`[drawAnnotations] Draw complete`);
  }, [history, historyIndex]);

  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Subscribing to range changes for redraw`);
    ...
    return () => {
      console.log(`[DraggableResizableChart] Unsubscribing from range changes`);
  ...
  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Setting up price change interval`);
    ...
    const interval = setInterval(() => {
      ...
      if (maxPrice === null || minPrice === null) {
        console.log(`[DraggableResizableChart] Invalid prices in interval: max=${maxPrice}, min=${minPrice}`);
        return;
      }

      if (maxPrice !== prevMaxPrice || minPrice !== prevMinPrice) {
        console.log(`[DraggableResizableChart] Price change detected: max=${maxPrice}, min=${minPrice}`);
    ...
    return () => {
      console.log(`[DraggableResizableChart] Clearing price change interval`);
      clearInterval(interval);
    };
  }, [drawAnnotations]);

  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Triggering drawAnnotations on currentTool change: currentTool=${currentTool}`);
    drawAnnotations();
  }, [drawAnnotations, currentTool]);

  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Setting up mouse event listeners: currentTool=${currentTool}`);
    ...
    const getCoords = (e) => {
      ...
      console.log(`[getCoords] Coords: x=${x}, y=${y}, time=${time}, value=${value}`);
      return { time, value };
    };

    const onMouseDown = (e) => {
      console.log(`[onMouseDown] Mouse down: currentTool=${currentTool}`);
      const coords = getCoords(e);
      if (currentTool === 'line' || currentTool === 'rectangle') {
        startPoint.current = coords;
        console.log(`[onMouseDown] Start point set: ${JSON.stringify(coords)}`);
      } else if (currentTool === 'text') {
        const text = prompt('Введите текст:');
        if (text) {
          console.log(`[onMouseDown] Adding text annotation: text=${text}, coords=${JSON.stringify(coords)}`);
          addAnnotation({ type: 'text', text, p1: coords });
        }
      } else if (currentTool === 'eraser') {
        const hit = findHitAnnotation(e.clientX, e.clientY);
        console.log(`[onMouseDown] Eraser hit: ${hit}`);
        if (hit !== null) {
    ...
    const onMouseMove = (e) => {
      ...
      console.log(`[onMouseMove] Mouse move while drawing: currentTool=${currentTool}`);
      ...
      const minPrice = candleSeries.current.coordinateToPrice(paneHeight);
      if (maxPrice === null || minPrice === null) {
        console.log(`[onMouseMove] Invalid prices: max=${maxPrice}, min=${minPrice}`);
        ctx.restore();
        return;
      }
      const topY = candleSeries.current.priceToCoordinate(maxPrice);
    ...
    const onMouseUp = (e) => {
      if (!startPoint.current) return;
      console.log(`[onMouseUp] Mouse up: adding annotation`);
      const coords = getCoords(e);
    ...
    const addAnnotation = (ann) => {
      console.log(`[addAnnotation] Adding: ${JSON.stringify(ann)}`);
    ...
    const removeAnnotation = (index) => {
      console.log(`[removeAnnotation] Removing index=${index}`);
    ...
    const findHitAnnotation = (clientX, clientY) => {
      console.log(`[findHitAnnotation] Checking hit: clientX=${clientX}, clientY=${clientY}`);
      const rect = canvas.getBoundingClientRect();
      ...
      for (let i = history[historyIndex].length - 1; i >= 0; i--) {
        ...
        if (x1 === null || y1 === null) {
          console.log(`[findHitAnnotation] Invalid coords for ann ${i}: x1=${x1}, y1=${y1}`);
          continue;
        }
        ...
        if (ann.type === 'text') {
          ...
          if (x >= x1 && x <= x1 + metrics.width && y >= y1 - 14 && y <= y1) {
            console.log(`[findHitAnnotation] Hit text at index=${i}`);
            return i;
          }
        } else {
          ...
          if (x2 === null || y2 === null) {
            console.log(`[findHitAnnotation] Invalid coords for ann ${i}: x2=${x2}, y2=${y2}`);
            continue;
          }
          ...
          if (ann.type === 'rectangle') {
            if (x >= minX && x <= maxX && y >= minY && y <= maxY) {
              console.log(`[findHitAnnotation] Hit rectangle at index=${i}`);
              return i;
            }
            ...
            if (distance <= 5 && bbHit) {
              console.log(`[findHitAnnotation] Hit line at index=${i}, distance=${distance}`);
              return i;
            }
          }
        }
      }
      console.log(`[findHitAnnotation] No hit`);
      return null;
    };
    ...
    return () => {
      console.log(`[DraggableResizableChart] Removing mouse event listeners`);
  ...
  React.useEffect(() => {
    console.log(`[DraggableResizableChart] Setting up keydown listener`);
    const handleKeyDown = (e) => {
      console.log(`[handleKeyDown] Key down: key=${e.key}, ctrl/meta=${e.ctrlKey || e.metaKey}`);
      if (e.ctrlKey || e.metaKey) {
        if (e.key === 'z') {
          if (historyIndex > 0) {
            console.log(`[handleKeyDown] Undo: newIndex=${historyIndex - 1}`);
            setHistoryIndex(historyIndex - 1);
            updateAnnotations(id, history[historyIndex - 1]);
          }
        } else if (e.key === 'y') {
          if (historyIndex < history.length - 1) {
            console.log(`[handleKeyDown] Redo: newIndex=${historyIndex + 1}`);
            setHistoryIndex(historyIndex + 1);
            updateAnnotations(id, history[historyIndex + 1]);
          }
        }
    ...
    return () => {
      console.log(`[DraggableResizableChart] Removing keydown listener`);
      window.removeEventListener('keydown', handleKeyDown);
    };
  }, [history, historyIndex, id, updateAnnotations]);

  const onResize = (event, { size }) => {
    console.log(`[onResize] Resizing: width=${size.width}, height=${size.height}`);
  ...
  const onResizeStop = (event, { size }) => {
    console.log(`[onResizeStop] Resize stop: width=${size.width}, height=${size.height}`);
  ...
  console.log(`[DraggableResizableChart] Drag state: isDragging=${isDragging}`);
  ...
  const handleToolClick = (tool) => {
    console.log(`[handleToolClick] Tool clicked: ${tool}`);
    if (tool === 'clear') {
...
const InfiniteGrid = () => {
  ...
  React.useEffect(() => {
    console.log(`[InfiniteGrid] Saving charts to localStorage: count=${charts.length}`);
    localStorage.setItem('charts', JSON.stringify(charts));
  }, [charts]);

  const moveChart = (id, left, top, newWidth, newHeight) => {
    console.log(`[moveChart] Moving chart: id=${id}, left=${left}, top=${top}, width=${newWidth}, height=${newHeight}`);
  ...
  const resizeChart = (id, width, height) => {
    console.log(`[resizeChart] Resizing chart: id=${id}, width=${width}, height=${height}`);
    setCharts((prev) => prev.map((chart) => (chart.id === id ? { ...chart, width, height } : chart)));
  };

  const updateAnnotations = (id, annotations) => {
    console.log(`[updateAnnotations] Updating annotations for id=${id}: length=${annotations.length}`);
  ...
  const [, drop] = useDrop({
    accept: CHART_TYPE,
    drop(item, monitor) {
      console.log(`[useDrop] Drop detected: item.id=${item.id}`);
  ...
  React.useEffect(() => {
    const handleResize = () => {
      console.log(`[InfiniteGrid] Window resize detected`);
      const viewportWidth = window.innerWidth;
      const viewportHeight = window.innerHeight;
      const cellWidth = Math.floor(viewportWidth / 2);
      const cellHeight = Math.floor(viewportHeight / 2);
    ...
    return () => {
      console.log(`[InfiniteGrid] Removing window resize listener`);
...
if (rootElement) {
  console.log(`[Root] Root element found, rendering app`);
