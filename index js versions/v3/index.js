...
const MAX_RAW_SIZE = CHUNK_SIZE * 4; // Max raw data to keep in memory, increased to 2M
  ...
  const updateVisibleData = () => {
    ...
    try {
      ...
      if (clampedFrom !== from || clampedTo !== to) {
        ...
        return; // Exit early, will trigger another range change
      }
      from = clampedFrom;
      to = clampedTo;
      ...
      if (effectiveMaxIndex < effectiveMinIndex) {
        console.log(`[updateVisibleData] Invalid effective range: clearing data`);
        updateChartData([]);
